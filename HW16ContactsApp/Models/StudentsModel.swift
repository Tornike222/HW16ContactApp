...
let maleAvatarNames = ["Avatar Image-4 Male", "Avatar Image-5 Male", "Avatar Image-6 Male", "Avatar Image-7 Male", "Satvaleebiani Tornike"]

//MARK: Objects of Student
...
let phoneOwner =  Student(name: "Tornike Elkanishvili", hobby: "Singing", gender: .male, age: 21, avatar: UIImage(named: maleAvatarNames[4])!)


//MARK: Functions for work with Student model
func sortedPrefixOf(studentArray: [Student]) -> [String]{
    var sortedChars = [String]()
    var uniqChar = Set<String>()
    
    for student in students {
        let firstChar = student.name.prefix(1)
        uniqChar.insert(String(firstChar))
    }
    sortedChars = uniqChar.sorted()
    return sortedChars
}

func getDictionary(from sortedChars: [String], via studentArray: [Student]) -> [String:[String]]{
    var studentDictionary = [String:[String]]()
    
    studentArray.forEach { student in
        let prefix = String(student.name.prefix(1))
        if studentDictionary[prefix] == nil {
            studentDictionary[prefix] = []
        }
        studentDictionary[prefix]?.append(student.name)
    }
